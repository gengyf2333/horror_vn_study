...
public class AIStoryJSON : MonoBehaviour
{
    ...
    //// --- Story Text Source ---
    public TextAsset InputFile;//This file can be in json or just in natural language(.txt
    ...
    public void ExportHistoryLines()
    {
        ...
        Debug.Log("The history record has been exported into the path: " + historyFilePath);

        // === copy GeneratedBackgrounds folder ===
        string sourceFolder = Path.Combine(Application.persistentDataPath, "GeneratedBackgrounds"); // 
        string targetFolder = Path.Combine(exportFolder, "GeneratedBackgrounds");

        if (Directory.Exists(sourceFolder))
        {
            CopyDirectory(sourceFolder, targetFolder);
            Debug.Log("The images have been copied into: " + targetFolder);
        }
        else
        {
            Debug.LogWarning("Not found GeneratedBackgrounds folder: " + sourceFolder);
        }
        ...
        try
        {
            ...
            Debug.Log("Succeed " + exportFolder);
        }
        catch (Exception e)
        {
            Debug.LogWarning("Cannot open successfully " + e.Message);
        }
#endif
        Debug.Log("Complete exporting！");
    }
    ...
    IEnumerator ProcessStoryWithAI()
    {
        ...
        string storyContent = InputFile.text;
    ...
    IEnumerator GetExpressionIndexRoutine(string historyContent, string speakerName, System.Action<int> callback)
    {
        ...
        // ===== Few-shot Prompt =====
        string prompt =
    $@"You are a visual novel character expression evaluator. 
You can only respond with a single digit from 0 to 7. 

Character: {speakerName}
Based on the current text, determine the most natural facial expression for this character.
...
Output rules:
- Only output a single digit (0, 1, ..., 7)
...
";
        ...
        {
            Debug.LogError("Wrong in expression API " + req.error);
            callback?.Invoke(11);  // 默认正常
            yield break;
        }

        string raw = req.downloadHandler.text;
        Debug.Log("Mini return: " + raw);

        try
        {
            var json = SimpleJSON.JSON.Parse(raw);
            string content = json["choices"][0]["message"]["content"].Value.Trim();
    ...
    void DisplayNextLine()
    {
        ...
        else
        {
            ...
            if (regularSpeaker == "Aoi")
            {
                ...
                //Aoi.SetBody(index);
                StartCoroutine(GetExpressionIndexRoutine(regularContent, regularSpeaker, (index) =>
                {
                    Aoi.SetBody(index);
                }));
                //Aoi.SetBody(index);
    ...
    void OnOptionSelected(int optionIndex)
    {
        ...
        ClearPreviousAIContent();//try clearing original generated content
    ...
    IEnumerator SendOptionToAI(string selectedOption)
    {
        ...
        storyTextDisplay.text = selectedOption + "...";

        // --- Build history string ---
        StringBuilder historyBuilder = new StringBuilder();
        foreach (string historicalLine in DialogueHistory)
        {
            historyBuilder.AppendLine(historicalLine);
        }
        string historyContext = historyBuilder.ToString().Trim();
        ...
        prompt += $"This is the full story script (JSON format or natural language):\n{storyContent}\n\n";
        prompt += $"The story is already in progress, not at the beginning. The player has just selected: \"{selectedOption}\"\n\n";
        prompt += "Based on the player's choice, please locate the current position in the script and generate all subsequent story content following this option, including the 'branch_focus' section.";

        // Few-shot examples
        prompt += "[Example]\n";
        prompt += "When the player selects \"Option A: Ally with Rylan\", the output should be:\n\n";
        prompt += "branch_focus: Kira decides to team up with Rylan, establishing a tentative partnership despite lingering distrust.\n\n";
        prompt += "Kira: Alright, let's work together. But trust is earned, not given.\n";
        prompt += "Kira felt a spark of hope; perhaps with Rylan's skills combined with hers, they could gather more information than she could alone.\n";
        prompt += "[End of Example]\n\n";

        prompt += "Please generate all remaining story content, including the ending. Your output:";

        lastPrompt = prompt; // Save the prompt
    ...
    void ClearPreviousAIContent() //clear the AI generated content
    {
        processedDialogueLines = new string[0];
        ...
        Debug.Log("VNStoryLoader: Previous AI generated content cleared. History content is preserved.");
    }
